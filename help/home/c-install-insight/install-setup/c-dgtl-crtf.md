...
1. Öppna webbläsaren i [!DNL https:\\license.visualsciences.com].

   >[!NOTE]
   >
   >Din webbläsare kan uppmana dig att presentera ett digitalt certifikat just nu. Om den gör det klickar du på **[!UICONTROL Cancel]** för att stänga dialogrutan.

1. På inloggningsskärmen anger du [!DNL Account Name] och [!DNL Password] som du har fått från Adobe och klickar sedan på **[!UICONTROL login]**.
1. Leta reda på certifikatet som har utfärdats för din Insight-instans ( *Ditt namn*.pem) och klicka på ikonen ![](assets/btn_save_certificatedownload.PNG) som är kopplad till certifikatet.
1. När du uppmanas att spara certifikatet klickar du på **[!UICONTROL Save]**.
1. Hämta filen till mappen [!DNL Certificates] i den katalog där du installerade Insight.
...
1. Du måste ha tillgång till filen [!DNL certmgr.msc] med möjlighet att importera ett certifikat och en nyckel till arkivet **Personal**. (Detta bör vara sant som standard för de flesta Windows-användare.)

1. Användaren som utför konfigurationen måste ha en kopia av kommandoradsverktyget **OpenSSL**.
1. Servern och klienten måste redan vara konfigurerade att använda ett anpassat SSL-certifikat, med instruktioner om hur klientcertifikatet ska lagras i Windows certifikatarkiv i stället för att lagras i katalogen **Certificates**.
...
Certifikatets namn är `<Common Name>.pem`, till exempel Analytics Server 1.pem (inte filen trust_ca_cert.pem.)

Innan certifikatet och den privata nyckeln kan importeras måste de konverteras från . [!DNL pem] till ett  [!DNL .pfx] format, t.ex.  [!DNL pkcs12.pfx] ).
...
1. Öppna **Personal** certifikatarkivet för den aktuella användaren.

   ![](assets/6_5_crypto_api_0.png)

1. Högerklicka på **Certifikat** och klicka på **Alla uppgifter** > **Importera**.

   Kontrollera att alternativet **Aktuell användare** är markerat och klicka sedan på **Nästa**.

   ![](assets/6_5_crypto_api_4.png)

1. Klicka på **Bläddra** och välj `<CommonName>.pfx`-filen som du skapade tidigare. Du måste ändra listrutan för filtillägg från ett X.509-certifikat till antingen **Personal Information Exchange** eller till **Alla filer** för att kunna se den.

   Markera filen och klicka på **Öppna** och sedan på **Nästa**.

1. Ange inget lösenord och kontrollera att endast alternativen **Markera den här nyckeln som exporterbar** och **Inkludera alla utökade egenskaper** är markerade.

   ![](assets/6_5_crypto_api_3.png)

   Klicka på **Nästa**.

1. Kontrollera att **Placera alla certifikat i följande butik** är markerat och att det listade certifikatarkivet är **Personligt**. (Om du är en avancerad användare kan du välja en annan butik just nu, men du måste ändra konfigurationen senare.)
...
1. Lägg till de här parametrarna i deras [!DNL Insight.cfg]-fil. Du kan göra detta från arbetsstationen eller manuellt genom att lägga till följande parametrar till [!DNL serverInfo]-objektet. (Se till att använda blanksteg i stället för tabbtecken och gör inga andra typografiska fel eller syntaxfel i den här filen. )
   ...
   >[!NOTE]
   >
   >Namnet &quot;Personal&quot; i certifikatshanteraren (certmgr.msc) refererar till certifikatarkivet **My.** Om du importerar SSL-kommunikationscertifikatet och nyckeln (.PFX) till det  **** personliga certifikatarkivet enligt rekommendation måste du ange  **SSL CryptoAPI Cert Store-** namnsträngen till &quot;My&quot;. Det går inte att ange den här parametern som &quot;Personlig&quot;. Detta är en egenskap i Windows certifikatarkiv.

   En fullständig lista över de fördefinierade systemarkiven finns här: [https://msdn.microsoft.com/en-us/library/windows/desktop/aa388136(v=vs.85).aspx](https://msdn.microsoft.com/en-us/library/windows/desktop/aa388136%28v=vs.85%29.aspx). Systemet kan ha ytterligare certifikatarkiv. Om du vill använda ett annat arkiv än Personligt (till exempel **Mitt**) måste du hämta det kanoniska namnet på certifikatarkivet och ange det i filen [!DNL Insight.cfg]. (Systemarkivnamnet &quot;My&quot; (Mitt) kallas inkonsekvent för &quot;My&quot; och &quot;MY&quot; i Windows-dokumentationen. Parametern verkar inte vara skiftlägeskänslig.)

1. När du har lagt till de här parametrarna och verifierat att värdena matchar listan i Windows certifikatshanterare, sparar du filen [!DNL Insight.cfg].
...
När ett certifikat inte hittas eller är ogiltigt genereras det här felmeddelandet till [!DNL HTTP.log]-filen.
...
>[!NOTE]
>
>L4-loggningsramverket kan aktiveras genom att du konfigurerar filen [!DNL L4.cfg] (se din kontohanterare för att konfigurera detta).
...
Ett certifikat som används av antingen Datans Workbench klient eller server måste signeras av en betrodd certifikatutfärdare (Certificate Authority). Data Workbench får certifikat som signerats av Visual Sciences CA. Dessa certifikat är pålitliga av Datan Workbench eftersom [!DNL trust_ca_cert.pem] (tillhandahålls tillsammans med Insight-programmet och lagras i katalogen **Certificates** för både  och klienter) innehåller ett *rotcertifikatutfärdarcertifikat* för Visual Sciences certifikatutfärdare. Dessa certifikat används både för licensiering av programvaran och autentisering när klienter och servrar kommunicerar med varandra med SSL. Endast certifikat som utfärdas av Visual Sciences CA kan användas för licensiering, men andra certifikat kan användas för kommunikation och autentisering. Certifikat som utfärdats av andra certifikatutfärdare än Visual Sciences kallas nedan *anpassade certifikat.*

**Viktigt:** För servrar och klienter använder Datan Workbench certifikatfiler som är installerade i klientens eller serverns  **** certifikatkatalog eller certifikat som uttryckligen identifieras i dess konfiguration. Du kan dock även använda Windows certifikatarkiv för klienter.
...
1. Lägg till certifikatet för den utfärdande certifikatutfärdaren i [!DNL trust_cert_ca.pem], som är installerat i katalogen **Certificates** för klienten och certifikatet för alla servrar i varje kluster som ska användas med det här anpassade certifikatet.
...
1. Hämta ett anpassat certifikat för varje server i klustret med följande villkor:
   ...
   1. Certifikatet utfärdades med *syfte **** av *klient* (eller både *server* **och** *klient*).
      ...
      För ett klientcertifikat krävs endast det andra kommandot för att ge [!DNL OK].

1. Placera certifikatet i klientens **certifikatkatalog**.
1. I [!DNL Insight.cfg] under *serverInfo* för varje kluster som du vill använda det här certifikatet kontrollerar du att *det anpassade klientcertifikatet* är namngivet, till exempel:
...
I det här avsnittet förutsätts att du har ett kluster som körs med certifikat som utfärdats av Visual Sciences och att konfigurationen följer vanliga rutiner (som katalogen *Components for Processing Servers* på överordnad synkroniseras med katalogerna *Components* för alla DPU:er).
...
1. Skaffa ett anpassat certifikat för varje server i klustret med följande krav:
   ...
   1. Certifikatet utfärdades med syftet *server* och *klient*.
...
1. Installera varje servers anpassade certifikat i katalogen **Certificates** på servern som [!DNL custom_communications_cert.pem].

1. Använd en textredigerare och lägg till följande rad i **Communications.cfg**-filen i både *Components* och *Components for Processing Servers*-katalogerna, direkt under den första raden ( [!DNL component = CommServer]):
...
När Insight-servern eller klienten söker efter ett **licens**-certifikat i katalogen **Certificates** försöker den validera alla certifikat (utom [!DNL trust_ca_cert.pem]) mot en hårdkodad kopia av Insight CA-certifikatet, som inte fungerar för något anpassat certifikat som finns i katalogen. Servern utfärdar den här varningen:
...
När du kommunicerar mellan Datans Workbench klient (arbetsstation) och server kan du spara en Value-parameter (till exempel ett lösenord) med typen *EncryptedString*. Parametern döljs och strängen sparas i *Windows Credential Store* på servern med motsvarande nyckel returnerad. Detta lagrar i första hand inloggningsuppgifter som används vid export, men kan användas för att kryptera alla parametrar.
...
* En ny konfigurationsfil har lagts till på Server\Component\**EncryptedStrings.cfg**.
   ...
   Den här filen hämtar mappen *Server*\*EncryptStrings* för krypteringskonfigurationsfiler.
...
1. Skapa en **EncryptedStrings.cfg**-konfigurationsfil för en sträng med följande fält:
   ...
   * *Värde*  - Det här fältet innehåller den vanliga textsträng som behöver krypteras.
      ...
      Detta är endast kryptering på serversidan. Inställningen *Värde* är bara krypterad på serverdatorn.
...
1. Spara filen **EncryptedStrings.cfg** till mappen Server\**EncryptStrings**.

**Utdatafil**

En utdatafil genereras med samma namn som indatafilen med ett &lt;*filnamn*>.** encrypteEXtension. Om indatafilen till exempel heter *sample.cfg* får utdatafilen namnet *sample.cfg.encrypted*.
