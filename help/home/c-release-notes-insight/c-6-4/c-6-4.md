...
| Ny cfg-fil för loggningsalternativ för ExportIntegration.exe | En ny   httpLoggingEI.cfg konfigurationsfil (finns på   server\Admin\Export\httpLoggingEI.cfg ) kan du stoppa INFO-loggning till   HTTP.log-fil under export av exportintegrering. (CRS-, TNT- och MMP-exporter används redan till att hämta utförlig loggning i enskilda exportloggfiler.)  En true-inställning startar INFO-loggning (för testning och detaljerad rapportering) till   HTTP.log-fil och en false-inställning stoppar utförlig loggning. Om inställningen är false skickas endast meddelanden på VARNING-/ERROR-nivå till   HTTP.log-fil. |
...
| Alternativ för datumformat | Du kan ändra datumformatet baserat på ditt språkområde i dialogrutan   Standardfilen Dimensions.cfg. Ändra standardformatet MM/DD/ÅÅÅÅ till formatet DD/MM/ÅÅÅÅ eller välj andra alternativ. |
