...
Så här konfigurerar du klustret: på överordnad [!DNL Insight Server]:
...
* Uppdatera [!DNL Synchronize.cfg] filen i katalogen Komponenter för bearbetning av servrar så att den pekar på överordnad [!DNL Insight Server].
...
Använd följande procedur för att lägga till de [!DNL Insight Servers’] vanliga namn och IP-adresser som bearbetas i adressfilen på överordnad [!DNL Insight Server]. (Även om adressfilen underhålls och administreras överordnad [!DNL Insight Server]används den av alla [!DNL Insight Servers] i klustret.)

>[!NOTE]
>
>Följande förutsätter att adressfilen redan har konfigurerats för överordnad [!DNL Insight Server]. Om du inte redan har lagt till de överordnad [!DNL Insight Server’s] IP-adresserna till adressfilen, ska du slutföra proceduren som beskrivs i [Definiera serverns nätverksplats](../../../../../../home/c-inst-svr/c-install-ins-svr/t-install-proc-inst-svr-dpu/c-svrs-ntwk-loc/c-svrs-ntwk-loc.md#concept-87dd2aa3448c415ca1285bc445a8c649) innan du börjar.
...
Du kan använda följande procedur för att konfigurera den centrala kopian av [!DNL Synchronize.cfg] filen. Den centrala kopian av filen finns på överordnad [!DNL Insight Server]. Bearbetningen [!DNL Insight Servers] i klustret initierar kommunikation med överordnad [!DNL Insight Server] för att hämta en uppdaterad kopia av den här filen.

Filen anger [!DNL Synchronize.cfg] platsen för överordnad [!DNL Insight Server]. Den identifierar också den uppsättning administrativa filer som varje bearbetning [!DNL Insight Servers] i klustret hämtar från överordnad [!DNL Insight Server]. Bearbetningen hämtar [!DNL Insight Servers] automatiskt dessa filer från överordnad [!DNL Insight Server] när de startas. De hämtar också dynamiskt uppdaterade kopior av dessa filer från överordnad [!DNL Insight Server] när filerna ändras.

>[!NOTE]
>
>Även om du konfigurerar [!DNL Synchronize.cfg] filen på överordnad [!DNL Insight Server]används [!DNL Insight Server] inte den här filen i själva överordnad. Du uppdaterar den här filen på överordnad [!DNL Insight Server] så att den är korrekt konfigurerad när bearbetningen [!DNL Insight Servers] hämtar filen.
...
1. Ange IP-adressen för den överordnad (primära) **[!UICONTROL Insight Server]** i parametern Klustrets primära serveradress.

   ![](assets/cfg_cluster_SyncFile_CentralCopy.png)

   Om du vill skapa en logg som registreras varje gång synkroniseringen sker mellan överordnad [!DNL Insight Server] och bearbetningen [!DNL Insight Servers]kontrollerar du att parametern Enable Synchronization Log är inställd på &quot;true&quot;.
