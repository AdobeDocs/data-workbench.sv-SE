...
Exemplet [!DNL Report Server.cfg] som visas i [Configuring the Connection to Insight Server](../../../home/c-rpt-oview/c-inst-rpt/t-config-conn-ins-svr.md#task-a3ca949c43244782b658fb4437fd724c) innehåller som standard bara de parametrar som finns i [!DNL Report Server.cfg] filen.

Om du kontaktar Adobe License Server via en proxyserver måste du lägga till licensvektorn och dess parametrar i [!DNL Report Server.cfg]. Här följer ett exempel på den här vektorn och dess parametrar som du kan använda som modell för din licensvektor:
...
Följande tabell innehåller beskrivningar av [!DNL Report Server.cfg] filparametrarna:
...
<table id="table_9DA4A3124A9D444CBB4CBFF6FA279A42"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Valfritt. Vektor som listar de teckensnitt som <span class="keyword"> Report Server </span> ska använda för att återge UTF8-baserade Unicode-specialtecken. Antalet teckensnitt i listan är obegränsat. </p> <p>Det första teckensnittet ska alltid vara Lucida Sans Console. Om den här parametern inte ingår i <span class="filepath"> Report Server.cfg- </span> filen, använder <span class="keyword"> </span> Report Server bara Lucida Sans-konsolen för att visa text. </p> <p> <span class="keyword"> Report Server </span> använder det första teckensnittet i listan för att återge alla tecken tills ett tecken som inte kan återges påträffas. Sedan används det andra teckensnittet i listan för att återge det tecknet. Om det teckensnittet inte återger tecknet <span class="keyword"> </span> använder Report Server det tredje teckensnittet för att återge tecknet och så vidare, tills det kommer till slutet av teckensnittslistan. Om rätt teckensnitt inte finns med i vektorn visas det hexadecimala värdet för tecknet i <span class="keyword"> </span> Report Server. </p> <p> <p>Obs!  Gör inga ändringar i den här parametern medan <span class="keyword"> Report Server </span> körs. </p> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Valfritt. Du behöver bara ändra parametrarna i den här vektorn om du kontaktar Adobes licensserver via en proxyserver. </p> <p>Avsnittsidentifierare för parametrar som du anger ska kontakta Adobes licensserver via en proxyserver. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Proxyadress </td> 
   <td colname="col2"> Adress till en proxyserver som <span class="keyword"> Report Server </span> måste använda för att få åtkomst till Adobes licensserver. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Proxylösenord </td> 
   <td colname="col2"> Valfritt. Lösenordet som är kopplat till <span class="wintitle"> proxyanvändarnamnet </span>. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> Nätverksplatsen som <span class="keyword"> Report Server </span> använder för att matcha serverns vanliga namn mot en IP-adress. Nätverksplatser definieras i adressfilen som finns i katalogen Addresses på servern data workbench. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Servrar </td> 
   <td colname="col2"> <p>Avsnittsidentifierare för parametrar som du anger för att konfigurera vilka data-workbench-serverdatorer som <span class="keyword"> Report Server </span> måste ansluta till för att generera rapporter. Detta inkluderar en siffra som anger hur många objekt som visas i den här vektorn. </p> <p>Lägg till en serverInfo-post för varje server och fyll i parametrarna efter behov. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> Proxyadress </td> 
   <td colname="col2"> Adressen till en proxyserver som <span class="keyword"> rapportservern </span> måste använda för att få åtkomst till data workbench-servern. Den här parametern behövs bara när en proxyserver krävs för att ansluta till serverdatorerna. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Proxylösenord </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> Namnet på SSL-certifikatfilen för <span class="keyword"> Report Server- </span> datorn. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> SSL-serverns namn </td> 
   <td colname="col2"> <span class="wintitle"> Namn på server </span> som anges på data workbench-serverns digitala certifikat. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Mängden minne (i kB) som du vill göra tillgängligt för rapporter och aviseringar. Standardvärdet är 50000. </p> <p>När du kör rapporter <span class="keyword"> kontrollerar </span> Report Server först värdet och kontrollerar sedan värdet i parametern Maximum Slice Size. Om du t.ex. anger parametern till 50 000 och den maximala segmentstorleken till 50, <span class="keyword"> </span> körs bara 50 arbetsytor i taget, även om det finns utrymme för att köra fler arbetsytor. </p> <p> <p>Obs!  Den här gränsen bör aldrig överstiga det värde som anges i data workbench-serverns Query Memory Limit-parameter och bör, helst, sättas lite lägre än Query Memory Limit (Gräns för <span class="wintitle"> frågeminne) </span> för att ge utrymme åt andra användare som kör rapporter samtidigt. </p> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Maximal segmentstorlek </td> 
   <td colname="col2"> Maximalt antal rapportarbetsytor som <span class="keyword"> Report Server </span> kan köras samtidigt. Standardvärdet är 50. Mer information om hur <span class="keyword"> Report Server </span> använder den här inställningen finns i <span class="wintitle"> parameterbeskrivningen för </span> Result Memory Limit (KB). </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Uppdatera programvara </td> 
   <td colname="col2"> <p>Anger om den här <span class="keyword"> rapportserverns </span> programvara ska kunna uppdateras av data workbench-servern. Alternativen är true eller false. Standardvärdet är true. </p> <p>Här följer ett exempel på den här parametern som du kan använda en modell: </p> <p> <code> Update&amp;nbsp;Software&amp;nbsp;=&amp;nbsp;bool:&amp;nbsp;false </code> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> E-postadressen som du vill skicka <span class="wintitle"> Report Server- </span> fel från. </td> 
  </tr> 
  <tr> 
