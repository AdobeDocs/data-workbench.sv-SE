...
Följande exempel innehåller bara de parametrar som ingår i [!DNL Insight.cfg] som standard.
...
Vissa av parametrarna som är tillgängliga i den nya [!DNL Insight.cfg] filen kanske inte är tillgänglig i din version av [!DNL Insight.cfg] -fil. Om en av dessa parametrar behövs måste du lägga till den i [!DNL Insight.cfg] fil använda [!DNL Add Custom Key]genom att högerklicka på en parameter och sedan ange namn och typ. Du kan också lägga till parametrar genom att öppna [!DNL .cfg] (finns i Datans Workbench installationskatalog) med en textredigerare.

Här följer ett exempel på alla parametrar som är tillgängliga för [!DNL Insight.cfg] fil som du kan använda som modell för att lägga till parameterposter:
...
Följande tabell innehåller beskrivningar av tillgängliga [!DNL Insight.cfg] filparametrar i alfabetisk ordning.
...
<table id="table_4465713A08B649E280A3B4840E829518"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Värdnamnet eller den numeriska IP-adressen för Datan Workbench. </p> <p>Exempel: <span class="filepath"> vsServer.mycompany.com eller 192.168.1.90</span> </p> <p>If <span class="wintitle"> Adress</span> har inte angetts, <span class="keyword"> klienten</span> använder det vanliga namnet som anges i parametern Gemensamt namn för SSL-servern när Använd adressfil är inställt på false. </p> <p> <p>Obs! Om parametern Använd adressfil är inställd på true kan texten som anges i parametern Address tas bort efter att den första profilen öppnas i <span class="keyword"> klienten</span>. Sedan avgör inställningen för parametern Nätverksplats vilka adresser från adressfilen för klustret som används för att ansluta till den överordnad servern. </p> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Färguppsättning </p> </td> 
   <td colname="col2"> <p>Anger bakgrundsfärgen för <span class="keyword"> klientprogram</span>. Alternativen är följande: </p> <p>0 = svart </p> <p>1 = vit </p> <p>2 = monokrom </p> <p>Standardvärdet är 0, svart. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Valfritt. Vektor som listar teckensnitten som <span class="keyword"> klienten</span> bör använda för att återge UTF8-baserade Unicode-specialtecken. Antalet teckensnitt i listan är obegränsat. </p> <p>Det första teckensnittet ska alltid vara Lucida Sans Console. Om den här parametern inte ingår i <span class="filepath"> Insight.cfg</span> file, Data Workbench använder bara Lucida Sans-konsolen för att visa text. </p> <p> Data Workbench använder det första teckensnittet i listan för att återge alla tecken tills det påträffas ett tecken som inte kan återges. Det använder det andra teckensnittet i listan för att återge det tecknet. Om det teckensnittet inte återger tecknet används det tredje teckensnittet i Datan Workbench för att återge tecknet, och så vidare, tills det kommer till slutet av teckensnittslistan. Om rätt teckensnitt inte finns med i vektorn visas det hexadecimala värdet för tecknet i Datan Workbench. </p> <p> <p>Obs! Ändra inte den här parametern medan Datan Workbench körs. </p> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Valfritt. Du behöver bara ändra parametrarna i licensvektorn om du kontaktar Adobe licensserver via en proxyserver. </p> <p>Avsnittsidentifierare för parametrar som aktiverar <span class="keyword"> klient</span> om du vill kontakta Adobe License Server via en proxyserver. Expandera licensieringsnoden och fyll i följande parametrar. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Proxyadress </p> </td> 
   <td colname="col2"> <p>Valfritt. Adress till proxyservern som <span class="keyword"> klient</span> måste använda för att få åtkomst till Adobe licensserver. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Anger den största tillåtna storleken i datacachen som används av <span class="keyword"> klienten</span> som körs på en enda dator. </p> <p>När parametern Sample Bytes finns i <span class="filepath"> Server.cfg</span> filen anger datacachestorleken för alla <span class="keyword"> klienter</span> som ansluter till en Data Workbench-server (250e6 byte som standard) kan du med parametern Maximal provstorlek ytterligare begränsa datacachestorleken för en viss dator. Detta är användbart när klienten är installerad på en dator med begränsad lagringskapacitet eller datorkraft. </p> <p> <p>Obs! Den här parametern begränsar storleken på ett lokalt dataexempel som efterfrågas lokalt av klientprogrammet. I motsats till <span class="filepath"> cache.db</span> filen innehåller data för varje profil som klienten ansluter till, plus elementnamnen och deras dimensioner. Dessa elementnamn och dimensioner i <span class="filepath"> cache.db</span> filer krävs för att köra lokala frågor. Det finns alltså inget annat sätt att begränsa dess storlek än att minska antalet element i datauppsättningen. </p> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Namn </p> </td> 
   <td colname="col2">Valfritt. Namnet som <span class="keyword"> klienten</span> används för att identifiera <span class="keyword"> server</span>. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Nätverksplats </p> </td> 
   <td colname="col2"> <p>Valfritt. Nätverksplatsen som <span class="keyword"> klient</span> används för att matcha Datans Workbench namn mot en IP-adress. De tillgängliga nätverksplatserna definieras i adressfilen på servern. Mer information finns i <i>Installations- och administrationshandbok för serverprodukter</i>. </p> <p>Om du inte anger någon nätverksplats <span class="keyword"> klient</span> löser vanliga namn med standardnätverksplatsen "Insight". </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Port </p> </td> 
   <td colname="col2"> <p>Den port till vilken <span class="keyword"> klienten</span> skickar begäranden. Portnumret måste matcha den port som Datan Workbench lyssnar efter begäranden på. Detta är vanligtvis 443 för säkra anslutningar. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Proxyadress </p> </td> 
   <td colname="col2"> <p>Om en proxyserver krävs för att ansluta till Datan Workbench måste du slutföra åtminstone den här parametern och proxyportparametern. Du kan också slutföra parametrarna Proxyanvändarnamn och Proxyanvändarlösenord. </p> <p>Adress till proxyservern som <span class="keyword"> klienten</span> måste använda för att komma åt Datan Workbench. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Proxylösenord </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Valfritt om du inte har fler än ett certifikat. Namnet på filen som innehåller det digitala certifikatet för den här kopian av Data Workbench. Det här är filen som du hämtade när du hämtade och installerade det digitala certifikatet. </p> <p>Exempel: <span class="filepath"> Samantha Smith.pem</span> </p> <p>Om du lämnar den här parametern tom <span class="keyword"> klienten</span> använder det certifikat som finns. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2">Anger om SSL används för säker kommunikation mellan Datan Workbench och <span class="keyword"> klienten</span>. Alternativen är true eller false. Standardvärdet är true. </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>Valfritt. Gör att du tillfälligt kan visa alla mått, dimensioner och filter som har dolts med någon av följande funktioner: 
     <ul id="ul_B40E28D9FDC0418BBFA9B0A37F4F6913"> 
      ...
      <li id="li_2ADE4EFC1F964D0A90B40CFB3D2766E8">Visa parameter i <span class="filepath"> .metric</span>, <span class="filepath"> .dim</span>och <span class="filepath"> .filter</span> filer. </li> 
      <li id="li_BBCD248A8F33440092B52E407E300FCC">Dold parameter i <span class="filepath"> Transformation.cfg</span> -fil. </li> 
     </ul> </p> <p>Mer information om de här metoderna finns i <a href="../../home/c-get-started/c-intf-anlys-ftrs/c-ctm-menus/t-cstm-menus-ordr-files.md#task-a391800a8dd444deb3e1516d5189f999"> Dölja ett menyalternativ</a>. </p> <p>Alternativen är true eller false. Standardinställningen är false. Ändra den här parametern till true om du tillfälligt vill visa dolda mått, dimensioner och filter. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Lås upp </p> </td> 
   <td colname="col2"> <p>Valfritt. Anger om du får låsa upp låsta arbetsytor. Alternativen är true och false. True gör att du kan låsa upp låsta arbetsytor, men false gör det inte. Standardvärdet är false. Mer information om låsta arbetsytor finns i <a href="../../home/c-get-started/c-work-worksp/c-unlock-wksp.md#concept-18ada952aecf45c79a806b31b294023e"> Låsa upp en arbetsyta</a>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Uppdatera programvara </p> </td> 
   <td colname="col2"> <p>Valfritt. Anger om detta ska tillåtas <span class="keyword"> den </span>klientprogramvara som ska uppdateras av Datan Workbench. Alternativen är true eller false. Standardvärdet är true. </p> </td> 
  </tr> 
  <tr> 
