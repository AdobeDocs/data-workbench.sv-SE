...
En [!DNL Categorize] omformning är i allmänhet enklare och snabbare än om du använder en [!DNL Regular Expression] omformning för att åstadkomma samma sak.

>[!NOTE]
>
>Det delsträngstest som används i [!DNL Categorize] är skiftlägeskänsligt om inte annat anges med [!DNL Case Sensitive] parametern.
...
I det här exemplet visas hur du använder omvandlingen för att integrera sökdata med händelsedata som samlats in från webbplatstrafiken. [!DNL Categorize] Anta att en viss webbplats har affärssektioner och att det finns ett krav på att kunna titta på och jämföra utifrån trafikflödet och det värde som genereras av de olika sektionerna. Du kan skapa en uppslagsfil som listar de delsträngar som används för att identifiera de olika avsnitten.
...
<table id="table_772B8ABF3B44493F99069010DDB5F77A"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> Indata </td> 
   <td colname="col2"> <span class="wintitle"> Kolumnnamn</span> är namnet på den kolumn som används för att matcha indata mot raden/raderna i filen. Om rubrikraden är true kan det vara namnet på en kolumn i sökfilen. Annars måste det vara det nollbaserade kolumnnumret som ska matchas mot. <span class="wintitle"> Fältnamn</span> är namnet på det fält som används för att hitta raden i sökfilen. </td> 
   <td colname="col3"> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Flera värden </td> 
   <td colname="col2"> <p>Sant eller falskt. Avgör om ett enskilt värde (en matchande rad) eller flera värden ska returneras (ett för varje matchande rad). </p> <p> <p>Obs!  Om <span class="wintitle"> Flera värden</span> är inställda på false måste du se till att det inte finns flera matchningar. När flera matchningar inträffar finns det ingen garanti för vilken matchning som returneras. </p> </p> </td> 
   <td colname="col3"> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Utdata </td> 
   <td colname="col2"> <p>En vektor med kolumnobjekt (resultat) där varje objekt definieras av kolumn- och fältnamn. </p> <p> <span class="wintitle"> Kolumnnamn</span> är den kolumn från vilken utdatavärdet hämtas. Om <span class="wintitle"> rubrikraden</span> är true kan det vara namnet på en kolumn i sökfilen. Annars måste det vara det nollbaserade kolumnnumret som ska matchas mot. </p> <p> <span class="wintitle"> Fältnamn</span> är namnet på det fält som används för att hämta utdata. Observera att detta kan vara en resultatvektor, en för varje rad som identifieras i det fall då parametern Multiple Values är true. </p> </td> 
   <td colname="col3"> </td> 
  </tr> 
 </tbody> 
</table>
...
I det här exemplet visas hur du använder omvandlingen för att integrera sökdata med händelsedata som samlats in från webbplatstrafiken. [!DNL FlatFileLookup] Anta att du vill isolera webbplatspartners som dirigerar trafik till webbplatsen och omvandla deras partner-ID:n till mer användarvänliga namn. Sedan kan du använda användarvänliga namn för att skapa utökade dimensioner och visualiseringar som tydligare kopplas till affärsrelationen än den relation mellan platser som används för att dirigera trafik.

Exempelomvandlingen söker i fältet cs(referrer-query) efter namnvärdepar för PartnerID, och om den finns [!DNL Lookups\partners.txt] används sökfilen för att jämföra PartnerID-värdet med värdena i [!DNL Partner] -kolumnen i tabellen. Om det finns en rad får utdatafältet x-partner-name namnet från den identifierade radens [!DNL PrintName] kolumn.
...
Omformningen [!DNL ODBCLookup] fungerar som en [!DNL FlatFileLookup] omformning. Den enda skillnaden är att den uppslagstabell som används under omvandlingen fylls i från en ODBC-databas och inte en platt fil.

>[!NOTE]
>
>[!DNL ODBCLookup] omvandlingar kan bara utföras under datauppsättningens omformningsfas. När det är möjligt rekommenderar Adobe att du använder omformningen i stället för [!DNL FlatFileLookup] [!DNL ODBCLookup] omformningen. [!DNL FlatFileLookup] omvandlingar är i sig mer tillförlitliga eftersom de inte är beroende av tillgängligheten för ett externt system. Dessutom är risken mindre att uppslagstabellen ändras om den finns i en platt fil som du kontrollerar lokalt.
...
<table id="table_B903DB291BCC4F44B09D54300216D288"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <span class="wintitle"> Kolumnnamn</span> är kolumnnamnet eller SQL-uttrycket för data som matchas mot indata. <span class="wintitle"> Fältnamn</span> är namnet på det fält som innehåller data som ska slås upp. </td> 
   <td colname="col3"> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Flera värden </td> 
...
* Parametrarna för datakällans namn, [!DNL Database User ID][!DNL Database Password]och tabell-ID är samma som parametrarna för samma namn som beskrivs för ODBC-datakällor. Se [ODBC-datakällor](../../../../home/c-dataset-const-proc/c-log-proc-config-file/c-odbc-data-sources.md#concept-5f2cf635081d44beab826ef5ec8cf4e3).

* Till skillnad från ODBC-datakällor kräver inte omformningar en ökande ID-kolumn. [!DNL ODBCLookup] Se [ODBC-datakällor](../../../../home/c-dataset-const-proc/c-log-proc-config-file/c-odbc-data-sources.md#concept-5f2cf635081d44beab826ef5ec8cf4e3). Det beror på att innehållet i uppslagstabellen inte får ändras på något sätt medan datauppsättningen är aktiv. Det går inte att identifiera ändringar i en uppslagstabell eller vy förrän omformningen görs. Mer information om hur du bearbetar data finns i [Återbearbetning och omformning](../../../../home/c-dataset-const-proc/c-reproc-retrans/c-unst-reproc-retrans.md).
...
I vårt exempel på omvandling söks loggposterna igenom efter s-dns-fältet, och om söktabellen finns används den för att jämföra s-dns-posten med posterna i tabellens [!DNL OLDDNS] kolumn. Om det finns en rad i tabellen får utdatafältet s-dns den uppdaterade DNS-postposten från den identifierade radens [!DNL NEWDNS] kolumn.
